...
def analyzeData(TBAdata: list[TBAMatch2026], scoutingData: list[MatchScouting2026]):
    ...
    print("scouting data length:", len(scoutingData))
    print("oprMatch rows:", len(oprMatchDataFrame))
    ...
    def create_fitness_func(min: float, max: float):
        def func(solution, functionInputs):
            solutionMatrix = solution.to_numpy(dtype=float)
            a = np.array(functionInputs[0], dtype=float)
            y = np.array(functionInputs[1], dtype=float)
    ...
    def perform_genetic_algorithm(i):
        ...
        for key in result[0].columns:
            if type(result[0][key].tolist()) is not None:
                results.append(np.array(result[0][key]).flatten())
    ...
    print("RATINGS FROM ANALYZE:", ratings)
    # print(XMatrix)
